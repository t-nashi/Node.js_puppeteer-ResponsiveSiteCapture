...
const getStatus = true                    // アクセスしたページのステータスゲットのオン・オフ
const saveStatusFileName = 'pageStatusExport.csv'

const getScreenshot = false               // アクセスしたページのscreenshotのオン・オフ
const screenSizeFull = true               // screenshotの画面サイズを全体にするかどうか

const DL = { waitUntil: 'domcontentloaded' } // ページの状態
...
// ** 対象のデバイスの種類を選択・コメントを解除 - end *****
  ...
  // ▼ 参考： Devices list
  // https://diff.intrinsic.com/puppeteer/1.12.2/1.15.0
...
let getTime = ''
let saveImgFileName = ''
let saveImgFileNameEnd = '_cap'              // getTime + '_' + i + [※※※ ココ ※※※] + extension
const urlLastElement = false                // urlの最後の「/スラッシュ」以降の文字列をファイル名末尾に付与するかどうか ※saveImgFileNameEndが上書きされる
const extension = '.png'
...
(async () => {
  ...
  for(i=0; i<arrUrls.length; i++){
    ...
    for(j=0; j<arrWidth.length; j++){
      ...
      // getStatusをするかどうか
      if(getStatus) {
        const response = await page.goto(arrUrls[i], DL);
        const result = (i+1) + '/' + arrUrls.length + ' ' +
                       response._status + ' ' +
                       response._url
        console.log(result)

        // ページのステータスをファイルに出力する
        // 何番目の指定URLか, ページステータス, URL
        const exportText = (i+1) + ',' + response._status + ',' + response._url

        // テキストファイル出力 - 上書き
        // writeFile(saveFileName, htmlData);

        // テキストファイル出力 - 追記
        // - 区切りver
        appendFile(saveStatusFileName, exportText + '\n')
        // - シンプルver
        // appendFile(saveFileName, getTextValue + '\n\n');
      }
      ...
      if(getScreenshot) {
        ...
        if(urlLastElement) saveImgFileNameEnd = getUrlLastElement(arrUrls[i])

        // ファイル名セット
        saveImgFileName = getTime + '_No' + (i+1) + '_' + j + extension

        // ページの高さ取得 → ページの全体キャプチャ
        // await page.waitForNavigation({waitUntil: 'networkidle2', timeout: pageWaitingTime})
        //    .catch(e => console.log('timeout exceed. proceed to next operation'));
        if(screenSizeFull) { await scrollToBottom(page, viewportHeight) }
        await page.screenshot({path: saveImgFileName, fullPage: screenSizeFull})
        console.log("save screenshot: " + saveImgFileName)
      }
      // -------------------- end（処理終了）
